...
in code under `app/src/main/java/com/vidsize/compressor/ui/`.

## 1. Design system

| Layer | File | Contents |
|---|---|---|
| Colour | `ui/theme/Color.kt` | `VidsizeColor` — every colour in the product, named. No hex literals in screens. |
| Spacing | `ui/theme/Dimens.kt` | `Space` (4→40dp scale, `gutter = 20dp`), `Radius` (14→28dp), `VidsizeShape`, `Sizes`. |
| Type | `ui/theme/Type.kt` | `VidsizeType` — 12 named styles on the platform sans family, each with ≥1.2× line height. |
| Theme | `ui/theme/VidsizeTheme.kt` | Material 3 `lightColorScheme` + shapes + typography. Light only, by decision. |
...
- Every panel is `VidsizeCard` — white, 1dp border, soft colour-matched shadow.
...
| App Open | Full-screen | Enabled with 3-day install grace, 3-session warm-up, 6-hour cooldown, 4-hour ad expiry, consent gate and workflow suppression. |
